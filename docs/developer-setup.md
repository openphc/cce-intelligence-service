...
> **Note:** The Intelligence Service depends on the Compliance Service schema (read-only tables: `protocol_definition`, `protocol_instance`, `step_instance`, `deviation`). Ensure the Compliance Service has run its Flyway migrations before starting.

> **Note:** The Compliance Service v1.0.0 does **not** yet publish `IntelligenceTriggerEvent` messages to `cce.intelligence.triggers`. The Intelligence Service can start and serve its REST API, but the Kafka consumer will not receive any triggers until the Compliance Service's producer is implemented in a future release. For local development, you can publish test triggers manually to the topic.
...
│       │   │   ├── readonly/    # ProtocolDefinition, ProtocolInstance, StepInstance, Deviation (immutable)
│       │   │   └── repository/  # JPA repositories for all entities
│       │   ├── engine/          # Intelligence processing pipeline
│       │   │   ├── RuleEvaluator.java        # JSONLogic condition evaluation
│       │   │   ├── ActionResolver.java       # Resolve definitionCanonical → ActionDefinition
│       │   │   ├── TemplateRenderer.java     # Render message templates with context
│       │   │   └── ActionDispatcher.java     # Route + deliver to ReceiverAdaptor
│       │   ├── kafka/           # Kafka consumer
│       │   │   ├── config/      # Consumer factory, topic bindings
│       │   │   ├── consumer/    # IntelligenceTriggerConsumer
│       │   │   └── model/       # IntelligenceTriggerEvent
│       │   ├── service/         # Business logic (IntelligenceEngine, ActionRunService, AuditService)
...
All CCE services share the same database (`cce_collector`) on the PostgreSQL instance deployed by the CCE Collector Service (port `5433`, user `cce_user`). The intelligence service owns 4 tables and reads 4 tables from the Compliance Service.

### 5.1 No Separate Database Creation Needed

The database is created by the collector service's Docker Compose. The intelligence service only runs its Flyway migrations on startup.

### 5.2 Prerequisite: Compliance Service Schema

The intelligence service reads from compliance-owned tables (`protocol_definition`, `protocol_instance`, `step_instance`, `deviation`). Ensure the compliance service has run its migrations before starting the intelligence service.
...
### 5.4 Current Migrations

| Version | Description | Script |
|---|---|---|
| V1 | Intelligence service schema (action_definition, action_run, receiver_adaptor, action_audit_log) | `V1__intelligence_schema.sql` |
...
./gradlew test --tests "org.openphc.cce.intelligence.engine.RuleEvaluatorTest"
