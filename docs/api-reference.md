...
**`cc` — its own dedicated env var (`CCE_OPSALERT_TIERn_CC`), same empty default for every tier.** `cc` is shaped exactly like `to`: one scalar env var per tier, accepting a single address or a comma-separated list, parsed the same way at both startup and send time. Unlike `to`, it has a default — but the same one for all three tiers, empty (`cc: "${CCE_OPSALERT_TIERn_CC:}"`, confirmed empirically to bind as a genuinely empty list — not a one-element list containing `""` — so an unconfigured tier adds no `Cc:` header at all, the email just goes to its `to`).

Tier 2 and 3 used to default to a nested-placeholder "escalation chain" instead — tier 2 auto-cc'ing tier 1's own address, tier 3 auto-cc'ing tiers 2 and 1's (`cc: "${CCE_OPSALERT_TIER2_CC:${ALERT_EMAIL_TIER1}}"`, etc.) — which worked, but turned out to be a real trap once exposed as an ordinary configurable env var through deploy-scripts: a chain default is only safe if every layer between Infisical and the app either supplies a real value or leaves the var genuinely *absent*. Both `docker-compose` interpolation and `envsubst` default an unset variable to an empty-but-*present* value, not absent — and Spring only falls back to a placeholder's own default when the property is entirely absent, not when it's present-but-empty. So the moment `CCE_OPSALERT_TIER2_CC` got wired into deploy-scripts at all, an operator simply forgetting to set it in Infisical would silently turn "cc tier 1" into "no cc," with no error anywhere. A flat empty default across all three tiers has no such trap: unconfigured always means no cc, configured always means exactly what was configured — confirmed live for both cases (no `CCE_OPSALERT_TIERn_CC` set at all → all three tiers send with no Cc; `CCE_OPSALERT_TIER2_CC`/`_TIER3_CC` set explicitly → those exact addresses show up as `RCPT TO` alongside each tier's `to`). If tier 2/3 should still cc an earlier tier's recipient, that's now just an explicit value in `CCE_OPSALERT_TIERn_CC`, the same as configuring any other recipient.
...
### `AlertTypeConfig.repeatIntervalMinutes` (`CCE_OPSALERT_REPEAT_INTERVAL_MINUTES`, default `1440` = 24h)

Once the tracker reaches the last configured tier (tier 3 for `INGESTION_GAP` today), `current_tier` never advances further — there's no tier 4 — so escalation genuinely stops there. This setting controls what happens to *notification*, independently: a positive value (the default, `1440`) makes `AlertEscalationEngine.attemptRepeat` keep resending that last tier's exact email — same recipients, same content — every `repeatIntervalMinutes`, until the incident resolves; `<= 0` turns that off — the engine goes fully silent past the last tier instead, same as before this feature existed. The default is on, not off, since daily repetition is the confirmed intended behavior for `INGESTION_GAP`, not an opt-in extra — set the env var explicitly only to change the cadence or to disable it.

**Measured from `last_notified_at`, not from the last tier's original send time — and not from `opened_at` either.** Confirmed by request: "daily 24 hours from the last mail sent," a rolling window from whichever send most recently happened (the last tier's first send, or an earlier repeat of it), not a fixed clock time or a count from the incident's true start. Concretely with the PRD's own numbers (tier 3 at 48h, `CCE_OPSALERT_REPEAT_INTERVAL_MINUTES=1440`): first repeat at 72h, second at 96h, and so on for as long as the incident stays `ACTIVE` — matching the same relative-timing philosophy tier 2/3 already use against tier 1 (see the class-level Escalation timing note on `AlertEscalationEngine`), just one level further out.

**Deliberately not implemented by reusing `attemptTier`.** That method's own guard — `row.currentTier() >= tier`, meaning "already sent, nothing to do" — is exactly backwards for a repeat: `current_tier` *never* changes once it reaches the last tier, so the same guard would permanently block every repeat attempt. `attemptRepeat` is its own method with its own condition ("how long since `last_notified_at`") instead, but still calls the existing `advanceTier` to record it — a same-value update to `current_tier` there, which is harmless, and it's what refreshes `last_notified_at` for the next repeat's own countdown.
