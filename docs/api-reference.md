...
### `AlertTypeConfig.repeatIntervalMinutes` (`CCE_OPSALERT_REPEAT_INTERVAL_MINUTES`, default `0` = disabled)

Once the tracker reaches the last configured tier (tier 3 for `INGESTION_GAP` today), `current_tier` never advances further — there's no tier 4 — so escalation genuinely stops there. This setting controls what happens to *notification*, independently: `<= 0` (the default) means the engine goes fully silent past the last tier, same as before this existed; a positive value makes `AlertEscalationEngine.attemptRepeat` keep resending that last tier's exact email — same recipients, same content — every `repeatIntervalMinutes`, until the incident resolves.
